...
public class JedisUtils {
	    ...
	    public JedisUtils() {
	    	/*if (pool == null) {
	            ...
	            //pool = new JedisPool(config, "10.141.111.185", 6379, 100000);
		...
		public static JedisUtils getRu(String redisHost, String redisPort) {
			if (pool == null) {
	            JedisPoolConfig config = new JedisPoolConfig();
	            config.setMaxTotal(10000);
	            // 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例。
	            ...
	            config.setMaxWaitMillis(1000 * 100);
	            config.setTestOnBorrow(true);
	            pool = new JedisPool(config, redisHost, Integer.valueOf(redisPort), 100000);
	        }
			return ru;
		}
