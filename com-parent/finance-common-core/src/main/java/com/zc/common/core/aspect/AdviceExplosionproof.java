...
public class AdviceExplosionproof {
	...
	@Autowired
	private RedisConfigrtion redisConfigrtion;
    ...
    public Object around(ProceedingJoinPoint point) throws Throwable {  
        ...
        JedisUtils jedisUtils = JedisUtils.getRu(redisConfigrtion.getHost(), redisConfigrtion.getPort());
        ...
        if(objSign == null || string.equals(objSign) || "".equals(objSign)){
        	return ResultUtils.returnError("请求异常");
        }
        Boolean num = jedisUtils.exists(objSign.toString());
        if(!num){
			//设置失效时间
    	    String setex = jedisUtils.setex(objSign.toString(), objSign.toString(), 10);
